...
export async function POST(req: NextRequest) {
    ...
    const r = await Salesforce.soql<{Id:string, Partner_ERP_ID__c:string}[]>(
        `SELECT Id, Partner_ERP_ID__c FROM Job__c WHERE Start_Date_Time__c >= ${start.toISOString()} AND End_Date_Time__c <= ${nextDay.toISOString()} AND Partner_ERP_ID__c <> '0' AND Partner_ERP_ID__c <> '1' AND Partner_ERP_ID__c <> 'null' AND Transmitted_To_Partner__c = '' AND Dispatch__c = 'chabe'`
    )
